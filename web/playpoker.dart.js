...
$$.Player = {"": ["name?", "human?", "hand=", "changing=", "handValue=", "pot", "status=", "encHand=", "gameInstance="],
 "super": "Object"
};
...
$$.Card = {"": ["r?", "s?", "suiteNames", "rankNames"],
 ...
 nameOfCard$0: function() {
  ...
  t2 = this.r;
  if (t2 !== (t2 | 0))
    return this.nameOfCard$0$bailout(1, t1, t2);
  if (t2 < 0 || t2 >= t1.length)
    throw $.ioore(t2);
  t4 = $.S(t1[t2]) + ' of ';
  t5 = this.suiteNames;
 ...
 nameOfCard$0$bailout: function(state0, t1, t2) {
  var t4, t5, t6;
  if (t2 !== (t2 | 0))
    throw $.iae(t2);
  if (t2 < 0 || t2 >= t1.length)
 ...
 cardImage$0: function() {
  ...
  if (0 >= t2.length)
    throw $.ioore(0);
  return 'web/cards/' + $.S($.toLowerCase(t2[0])) + $.S(this.r) + '.png';
},
 cardImage$0$bailout: function(state0, t2) {
  return 'web/cards/' + $.S($.toLowerCase($.index(t2, 0))) + $.S(this.r) + '.png';
},
 operator$gt$1: function(other) {
  var t1 = this.r;
  if ($.gtB(t1, other.get$r()))
    return true;
  else if (t1 === other.get$r())
 ...
 operator$lt$1: function(other) {
  var t1 = this.r;
  if ($.ltB(t1, other.get$r()))
    return true;
  else if (t1 === other.get$r())
    if ($.ltB(this.s, other.get$s()))
      return true;
  return false;
},
 operator$eq$1: function(other) {
  if (this.r === other.get$r() && $.eqB(this.s, other.get$s()))
    return true;
  return false;
}
};
...
$$.PokerEngine = {"": ["servedCards"],
 ...
 serveHand$1: function(player) {
  var t1, inCard;
  for (t1 = this.servedCards; $.ltB($.get$length(player.get$hand()), 5);) {
    inCard = this.pickCard$0();
 ...
 pickCard$0: function() {
  var rnd, randomSuite, t1;
  rnd = $.Random_Random(null);
  randomSuite = rnd.nextInt$1(4);
  t1 = rnd.nextInt$1(13);
  if (typeof t1 !== 'number')
    throw $.iae(t1);
  return $.Card$(randomSuite, 2 + t1);
},
 beenServed$1: function(toAdd) {
  var t1;
  for (t1 = $.iterator(this.servedCards); t1.get$hasNext() === true;)
    if ($.eqB(t1.next$0().nameOfCard$0(), toAdd.nameOfCard$0()))
      return true;
  return false;
},
 evaluateHand$1: function(player) {
  var cardsMultiplicity, t1, t2, t3, encMultiplicity;
  cardsMultiplicity = $.Map_Map();
  for (t1 = $.iterator(player.get$hand()); t1.get$hasNext() === true;) {
    t2 = t1.next$0();
    t3 = $.index(cardsMultiplicity, t2.get$r()) == null;
    t2 = t2.get$r();
    if (t3)
      $.indexSet(cardsMultiplicity, t2, 1);
    else
      $.indexSet(cardsMultiplicity, t2, $.add($.index(cardsMultiplicity, t2), 1));
  }
  for (t1 = $.iterator(cardsMultiplicity.get$values()), encMultiplicity = 0; t1.get$hasNext() === true;)
 ...
 allOnes$1: function(player) {
  var R, t1;
  R = [$.index(player.get$hand(), 0).get$r(), $.index(player.get$hand(), 1).get$r(), $.index(player.get$hand(), 2).get$r(), $.index(player.get$hand(), 3).get$r(), $.index(player.get$hand(), 4).get$r()];
 ...
 isFlush$1: function(player) {
  var allSuites, i, t1;
  allSuites = [$.index(player.get$hand(), 0).get$s(), $.index(player.get$hand(), 1).get$s(), $.index(player.get$hand(), 2).get$s(), $.index(player.get$hand(), 3).get$s(), $.index(player.get$hand(), 4).get$s()];
  for (i = 0; i < allSuites.length - 1;) {
    t1 = allSuites[i];
    ++i;
    if (!$.eqB(t1, allSuites[i]))
      return false;
  }
  return true;
},
 changeCards$1: function(player) {
  var t1, t2, rm;
  for (t1 = $.iterator(player.get$changing()); t1.get$hasNext() === true;) {
    t2 = t1.next$0();
    rm = $.indexOf$1(player.get$hand(), t2);
 ...
 compareHighCard$2: function(player, computer) {
  var playerMax, computerMax, t1, playerMax0, computerMax0;
  playerMax = $.Card$(0, 0);
  computerMax = $.Card$(0, 0);
  for (t1 = $.iterator(player.get$hand()); t1.get$hasNext() === true;) {
    playerMax0 = t1.next$0();
    if ($.gtB(playerMax0, playerMax))
      playerMax = playerMax0;
  }
  for (t1 = $.iterator(computer.get$hand()); t1.get$hasNext() === true;) {
    computerMax0 = t1.next$0();
 ...
 comparePair$2: function(player, computer) {
  var t1, playerPair, t2, t3, t4, t5, computerPair;
  t1 = {};
  playerPair = $.Map_Map();
  for (t2 = $.iterator(player.get$hand()); t2.get$hasNext() === true;) {
    t3 = t2.next$0();
    t4 = $.index(playerPair, t3.get$r()) == null;
    t5 = t3.get$r();
    if (t4)
      $.indexSet(playerPair, t5, [t3]);
    else
      $.add$1($.index(playerPair, t5), t3);
  }
  computerPair = $.Map_Map();
  for (t2 = $.iterator(computer.get$hand()); t2.get$hasNext() === true;) {
    t3 = t2.next$0();
    t4 = $.index(computerPair, t3.get$r()) == null;
    t5 = t3.get$r();
    if (t4)
      $.indexSet(computerPair, t5, [t3]);
    else
      $.add$1($.index(computerPair, t5), t3);
  }
  t1.playerPairRank_1 = null;
  t1.playerPairValue_2 = [];
  $.forEach(playerPair, new $.PokerEngine_comparePair_anon(t1));
  t1.computerPairRank_3 = null;
  t1.computerPairValue_4 = [];
  $.forEach(computerPair, new $.PokerEngine_comparePair_anon0(t1));
  if ($.gtB(t1.playerPairRank_1, t1.computerPairRank_3))
    ...
    t2 = $.iterator(t1.playerPairValue_2);
    while (t2.get$hasNext() === true) {
...
$$.main_function = {"": ["player_2", "toggled_1", "computer_0"],
 "super": "Closure",
 call$1: function(event$) {
  $.newHand(this.player_2, this.computer_0, this.toggled_1);
}
};

$$.main_function0 = {"": ["player_5", "toggled_4", "computer_3"],
 "super": "Closure",
 call$1: function(event$) {
  $.manageShowdownButton(this.player_5, this.computer_3, this.toggled_4, true, null);
}
};
...
$$.initCardSelection_function = {"": ["box_0", "toggled_4", "slots_3", "player_2"],
 "super": "Closure",
 call$1: function(event$) {
  $.toggleCard(this.player_2, this.box_0.i_10, this.toggled_4, this.slots_3);
}
};
...
$.main = function() {
  ...
  toggled = $.initCardSelection(player);
  $.newHand(player, computer, toggled);
  $.add$1($.query('#newHand').get$on().get$click(), new $.main_function(player, toggled, computer));
  $.add$1($.query('#showdown').get$on().get$click(), new $.main_function0(player, toggled, computer));
};

$.init = function(player, computer) {
  var t1, t2;
  for (t1 = $.iterator(['c1', 'c2', 'c3', 'c4', 'c5', 'h1', 'h2', 'h3', 'h4', 'h5']); t1.get$hasNext() === true;) {
    t2 = $.query('#' + $.S(t1.next$0())).get$style();
    t2.set$backgroundImage('url(web/img/deck.png)');
    t2.set$width('71px');
    t2.set$height('96px');
  }
};
...
$.newHand = function(player, computer, toggled) {
  ...
  t1 = $.updateStatus(player, computer, game);
  $.query('#status').set$innerHTML(t1);
  player.set$gameInstance(game);
  $.manageShowdownButton(player, computer, toggled, false, game);
  for (i = 0; i < 5; ++i)
...
$.initCardSelection = function(player) {
  var slots, toggled, i, box_0, box_00;
  slots = $.Map_Map();
  toggled = $.Map_Map();
  $.indexSet(slots, 0, 'h1');
  $.indexSet(slots, 1, 'h2');
  ...
  for (i = 0; i < 5; ++i)
    $.indexSet(toggled, i, false);
  box_0 = {};
  for (box_0.i_10 = 0; $.ltB(box_0.i_10, $.get$length(slots)); box_00 = {}, box_00.i_10 = box_0.i_10, box_00.i_10 = $.add(box_00.i_10, 1), box_0 = box_00)
    $.add$1($.query('#' + $.S($.index(slots, box_0.i_10))).get$on().get$click(), new $.initCardSelection_function(box_0, toggled, slots, player));
  return toggled;
};

$.toggleCard = function(player, slot, toggled, slots) {
  var t1, t2, t3, margin, rmIndex;
  if ($.ltB(player.get$status(), 2)) {
    $.indexSet(toggled, slot, $.index(toggled, slot) !== true);
    t1 = $.index(toggled, slot) === true;
    t2 = player.get$hand();
    t3 = player.get$changing();
    if (t1) {
      $.add$1(t3, $.index(t2, slot));
      margin = '-15px';
    } else {
      rmIndex = $.indexOf$1(t3, $.index(t2, slot));
  ...
  $.manageShowdownButton(player, player, toggled, false, null);
};
...
$.showCards = function(player) {
  var prefix, slots, i, t1, t2, t3;
  prefix = player.get$human() === true ? 'h' : 'c';
  slots = ['1', '2', '3', '4', '5'];
  for (i = 0; i < 5; ++i) {
    t1 = 'url(' + $.S($.index(player.get$hand(), i).cardImage$0()) + ')';
    t2 = '#' + prefix;
    if (i >= slots.length)
      throw $.ioore(i);
    t3 = $.query(t2 + $.S(slots[i])).get$style();
...
$.manageShowdownButton = function(player, computer, toggled, clicked, game) {
  var button, t1;
  button = $.query('#showdown');
  if ($.gtB($.get$length(player.get$changing()), 0)) {
    button.set$classes(['btn', 'btn-success']);
    button.set$text('Change cards and showdown');
  } else {
    button.set$classes(['btn', 'btn-info']);
    button.set$text('Showdown');
  }
  if (clicked === true) {
    game = player.get$gameInstance();
    if ($.gtB($.get$length(player.get$changing()), 0)) {
      game.changeCards$1(player);
      $.resetCardsPosition(toggled);
      game.showdown$2(player, computer);
    } else {
      $.resetCardsPosition(toggled);
      game.showdown$2(player, computer);
    }
    ...
    t1 = $.updateStatus(player, computer, game);
    $.query('#status').set$innerHTML(t1);
    button.get$style().set$visibility('hidden');
...
$.Player$ = function() {
  return new $.Player(null, null, [], [], null, 10000, 0, null, null);
};
...
$.CTC5 = new Isolate.$isolateProperties.ConstantMap(0, {}, Isolate.$isolateProperties.CTC1);
...
//@ sourceMappingURL=playpoker.dart.js.map